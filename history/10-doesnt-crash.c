...
int main(int argc, char **argv)
{
    ...
    SEL scheduleNotificationSelector = sel_registerName("scheduleNotification:");

    Method scheduleNotification = class_getInstanceMethod(
        NSUserNotificationCenter,
        scheduleNotificationSelector);

    // objc_property_t _Nullable z = class_getProperty(NSUserNotificationCenter, "defaultUserNotificationCenter");
    ...
    CFStringRef title = CFSTR("Test title");

    object_setInstanceVariable(userNotification, "title", (void *_Nullable)title);
